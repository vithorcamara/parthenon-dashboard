...
Os módulos aqui são **totalmente agnósticos à lógica de negócio**. Eles fornecem os sistemas fundamentais sobre os quais a aplicação é construída. Por exemplo, o `screenManager` sabe como trocar de cena, mas não sabe *o que é* uma `GameListscreen`.
...
- **screenManager.js**: Controla a transição entre as diferentes cenas (telas) da aplicação.
